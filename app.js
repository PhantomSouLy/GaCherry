...
function resolveCraftCard(query) {
  ...
  conconst DATA_URL = './data/cards.json';
const GUIDE_URL = './data/guide.json';
const NEWS_URL = './data/news.json';
...
function chip(filter, value, label) {
  return `
    <button class="chip ${state.filters[filter] === value ? 'active' : ''}" data-filter="${filter}" data-value="${esc(value)}">
      ${esc(label)}
    </button>
  `;
}
...
function renderCraft() {
  ...
  `).join('') : 'st direct = db.cards.find(card => searchKey(card.name) === key);
